classdef Freq < handle
    ...
    methods % public
        ...
        function [f] = synthFull(o, part)
            ...
            % synthesize full spectrum
            f = o.fftsynth(part, o.fr(1), o.full);
    ...
    methods (Static)
        ...
        function [ spect_full ] = fftsynth(...
                spect_partial,freq_low,freqs_full,dim)
            % Expand signal specified with only positive frequencies to
            % include the full set of positive/negative bins. The output
            % can be then passed to ifft to produce a real signal.
            %
            % INPUT spect_partial : input signals (selected contiguous
            % bins)
            %
            % INPUT freq_low : lowest frequency in spect_partial. if
            % specified as a vector, only the first element is used.
            %
            % INPUT freqs_full : full set of frequency bins to output (see
            % fftinfo)
            %
            % INPUT dim dimension along which signals are laid out. if
            % signals are in columns, dim is 1. default is 1.
            %
            % OUTPUT spect_full : signal including full set of frequency
            % bins
